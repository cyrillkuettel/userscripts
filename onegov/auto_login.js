...
(function() {
    ...
    function didLastPageloadFailWith500() {
        // Was the last page loaded with a 500? (Not an unusual occurence in development)
        // in that case don't redirect to login


        function isPageEssentiallyEmpty() {
            // Checks if the current DOM is essentially empty
            const bodyElements = document.body.children.length;

            // Check if there's meaningful text on the page
            const hasText = document.body.textContent.trim().length > 20;
            return bodyElements <= 2 && !hasText;
        }

        if (isPageEssentiallyEmpty()) {
            console.log('isPageEssentiallyEmpty
                return true;
            }
            ...
            if (window.performance) {
                ...
                if (navigation && navigation.responseStatus >= 500) {
                    console.log('[OneGov Helper] Detected 500 error via Performance API');
                    return true;
                }
            }
            return false;
